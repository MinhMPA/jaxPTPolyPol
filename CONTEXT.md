...
**Units (Stream-B layer-1 row):** their c1 is in [Mpc/h]² (counterterm ~ c1·k²μ²·…); ours dimensionless via (k/0.3)². Naive scaling: width 5 [Mpc/h]² → ≈ 5·k_NL² = 0.45 dimensionless, but derive the exact factor from CLASS-PT's operator (possible extra f/b1 factors) before use.
...
**Two-layer convention map (the high-risk item):** (1) a *constant* per-parameter factor converting our raw coefficient definition (e.g. counterterm `−2k²c0·P`) to the paper's, in matching units; (2) the *θ_NL-dependent* A_AP·A_amp rescaling above. Layer 1 must be derived by matching the coefficient definitions in ps_1loop_jax vs the paper/CLASS-PT — error here silently biases every posterior.
