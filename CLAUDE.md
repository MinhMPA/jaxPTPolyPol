...
- **Use `command grep` for any completeness claim — the shell `grep` lies here.**
  ...
  (`.superpowers/` among them), returning a NO-MATCH result (exit 1, empty output)
  indistinguishable from a genuine absence — during the fisher_joint CMB port it
  produced a first, false-empty repo-wide sweep for stale σ(m_ν) upper limits.
  Never conclude "no occurrences anywhere" from bare `grep`: use
