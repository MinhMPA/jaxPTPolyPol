...
# k-grid, quadrature, PFS_EMULATOR, META) are imported from stream_common (single
# source of truth, in lockstep with taylor_surrogate_validation.py and
# build_taylor_templates_lcdm.py).
# ---------------------------------------------------------------------------

BAO_DATA_DIR = DEFAULT_BAO_DATA_DIR   # run from example/mcmc
